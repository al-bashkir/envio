...
/// Print shell directives for the profile to stdout.
/// Wrap with `eval "$(envio load <profile>)"` (bash/zsh) or
/// `envio load <profile> | source` (fish).
#[cfg(target_family = "unix")]
pub fn load_profile(profile: Profile) -> Result<()> {
    let shell = crate::utils::detect_shell();
    let mut out = std::io::stdout().lock();
    write_exports(&mut out, &profile, shell)?;
    Ok(())
}
...
/// Print shell directives that unset every variable in the profile to stdout.
/// Wrap with `eval "$(envio unload <profile>)"` (bash/zsh) or
/// `envio unload <profile> | source` (fish).
#[cfg(target_family = "unix")]
pub fn unload_profile(profile: Profile) -> Result<()> {
    let shell = crate::utils::detect_shell();
    let mut out = std::io::stdout().lock();
    write_unsets(&mut out, &profile, shell)?;
    Ok(())
}
